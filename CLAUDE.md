...
pnpm run test:coverage # Vitest with V8 coverage (thresholds: lines 65%, branches 55%, functions 68%, statements 66%)
...
**Quality gate (matches CI `quality` job):** `pnpm run lint && pnpm run i18n:check && pnpm run typecheck && pnpm exec vitest run --coverage`. Full pipeline graph: [`docs/CI.md`](docs/CI.md). Coverage thresholds: lines 65, branches 55, functions 68, statements 66 (P1 target, see `vitest.config.ts`).
