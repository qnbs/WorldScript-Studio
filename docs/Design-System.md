...
- **Editorial workspace, not AI dashboard** (DS-6, Visual Maturity remediation): solid semantic surfaces are the default; glass/blur is an explicit, rare, transient effect, not a primitive default. Elevation is tied to real layer hierarchy via `--sc-elevation-*` (flat surfaces carry little/no shadow; only popovers/modals carry real elevation), and radius via `--sc-radius-*` expresses intent (compact control vs. panel vs. rare hero) rather than one soft radius everywhere. Ambient decoration (Aurora/noise) is scoped to the Welcome Portal brand moment (`body.portal-active`), not mounted for every product view. PRs A–E (#810–#815) de-glassed the shared primitives (Card, Button, Input, Textarea, Select, Checkbox, RadioGroup), overlays (Toast, BottomSheet), product identity (Header, Welcome Portal, Dashboard), and loading/trust surfaces (Skeleton, EmptyState, PWA notifications) by this rule. `scripts/audit-tokens.mjs`'s `ambient-glass-token` rule (PR F) now mechanically flags every raw `--glass-*` reference against a disclosed baseline (`token-audit-baseline.json`) so the pattern can't silently regrow; the baseline's remaining count is pre-existing feature-level usage (settings panels, writing tools, the language selector) the program hasn't reached yet, tracked as a known follow-up rather than fixed here.
