...
| C-3 | Medium | Typography | Fluid type capped at 1280px while line length grows unbounded without a content cap | `index.css:46-54` (`--text-sc-*` clamp ceiling) + no max-width | width cap (D1) + optional fluid tuning (D2) | 🟡 D1 width cap landed; fluid-type tuning deferred to D2 |
