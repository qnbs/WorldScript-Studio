...
| C-6 | Medium | Tauri/perf | Heavy glassmorphism (`backdrop-blur` in 24 files, incl. shared Input/Textarea/Checkbox/RadioGroup/Card/Modal/Drawer/Toast primitives — count re-verified 2026-08, doc previously said 47) — GPU cost on WebView2/WebKitGTK | `--glass-*` tokens; pervasive `backdrop-blur-*` | **Shipped (#332/D4):** `prefers-reduced-transparency` OS block now also strips direct `backdrop-blur-*` utilities via `[class*="backdrop-blur-"]` (previously only swapped `--glass-*` tokens); new manual "Reduce transparency effects" toggle (Settings › Accessibility, `accessibility.reducedTransparency`, default off) covers desktops/DEs that don't expose the OS preference | ☐ confirm — code fix shipped; actual GPU-cost relief still needs a packaged `.deb` measurement (see `docs/ISSUES-332-333-PERFORMANCE-LEDGER.md`) |
