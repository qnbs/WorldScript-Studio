...
| C-6 | Medium | Tauri/perf | Heavy glassmorphism (`backdrop-blur` in 47 files + `--glass-*`) — GPU cost on WebView2/WebKitGTK | `--glass-*` tokens; pervasive `backdrop-blur-*` | reduce blur on desktop / `reduceTransparency` setting (D4) | 🟡 D4 — `@media (prefers-reduced-transparency: reduce)` makes the token-driven `--glass-*` layer opaque on desktop. **Remaining:** consolidate the 47 files' raw `backdrop-blur-*` utilities onto the token layer + measure paint (needs the perf pass) |
| C-7 | Medium | Tauri/menu | Native menu is only File (Export/Settings/Quit) + Help — no Edit/View/Window | `src-tauri/src/lib.rs:install_app_menu` | enrich native menu (D4) | ✅ D4 — added **Edit** (predefined undo/redo/cut/copy/paste/select-all, OS-routed), **View** (Command Palette → `global-open-command-palette`), **Window** (minimize/maximize/fullscreen/close). Verify via `tauri-build.yml` (no Rust PR-CI gate) |
| C-8 | Med | Tauri/engine | Per-engine CSS quirks (scrollbars, `backdrop-filter`, `100dvh`, focus ring) unverified across WebView2/WebKit/WebKitGTK | global CSS | per-OS `.is-desktop[data-os]` fixes (D4) | 🟡 D4 — desktop scrollbars sized for a mouse (10px) + `scrollbar-gutter: stable` (all engines are Chromium/WebKit, so `::-webkit-scrollbar` covers them). Focus ring uses `focus-visible:ring-2` (engine-agnostic). Full `100dvh`/per-OS verification awaits the screenshot matrix |
