...
| `tauri-plugin-*` (Cargo) and `@tauri-apps/plugin-*` (npm) | Coupled via a top-level `multi-ecosystem-groups.tauri-plugins` entry (2026-09-10): both ecosystem entries tag their plugin packages (`patterns: ['tauri-plugin-*']` on the cargo entry, `patterns: ['@tauri-apps/plugin-*']` on the npm entry, each with `multi-ecosystem-group: tauri-plugins`) so Dependabot raises **one consolidated PR** when both sides have a matching available update, instead of two separate, uncoordinated PRs — this exact separation caused v1.28.5's tag-triggered release build to fail on every platform (#661 bumped only the Rust side; `tauri build` hard-rejects a Rust/npm major.minor mismatch). The cargo `tauri-deps` group (`tauri*`/`tauri-*`/`wry`/`tao`) gained an `exclude-patterns: ['tauri-plugin-*']` so a plugin crate never double-joins both groups. Verified against GitHub's official `dependabot-2.0.json` JSON Schema via `ajv` before merging. Grouping only *reduces the probability* of a lopsided PR (e.g. it can't help when only one side has an upstream update at all) — it does not replace the fail-closed authority: `pnpm run tauri-plugins:check` (`scripts/check-tauri-plugin-versions.mjs`) still runs unconditionally in `ci.yml`'s `quality` job on every PR and fails closed on any mismatch; `tauri-build.yml` additionally runs the same check as a cheap `parity-preflight` job before its ~45min cross-platform bundle matrix. If a Dependabot PR (grouped or not) still fails this check, the fix must land **on that same PR's branch** (Dependabot PRs accept additional pushed commits) — a separate companion PR cannot make the failing PR's own CI pass, since each PR's CI only sees its own branch; never dismiss or loosen the check itself. |
