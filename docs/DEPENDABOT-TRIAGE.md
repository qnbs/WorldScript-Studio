...
| `tauri*` / `wry` / `tao` (Cargo) and `@tauri-apps/plugin-*` (npm) | The Cargo-side `tauri-deps` group only covers Rust crates — Dependabot cannot group across the `npm` and `cargo` ecosystems, so a `tauri-plugin-*` Rust bump and its npm `@tauri-apps/plugin-*` counterpart always arrive as two separate, uncoordinated PRs (or just one side, if only one crate/package actually changed upstream). This exact gap caused v1.28.5's tag-triggered release build to fail on every platform (#661 bumped only the Rust side via Dependabot; `tauri build` hard-rejects a Rust/npm major.minor mismatch). **This is caught automatically:** `pnpm run tauri-plugins:check` (`scripts/check-tauri-plugin-versions.mjs`) runs unconditionally in `ci.yml`'s `quality` job on every PR — including a Cargo-only Dependabot PR — and fails closed on a mismatch; `tauri-build.yml` additionally runs the same check as a cheap `parity-preflight` job before its ~45min cross-platform bundle matrix. If a Cargo `tauri-deps` Dependabot PR fails this check: **bump the matching `@tauri-apps/plugin-*` npm package(s) to the version the failure message names, in the same PR** (or a same-day companion PR) — never dismiss or loosen the check itself. |
