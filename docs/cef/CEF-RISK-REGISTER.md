...
| R-10 | Documentation drift (stale Tier-A docs more dangerous than missing ones) | High | MITIGATING | *unassigned* | Ownership manifest (`OWNERSHIP.yaml`) established Wave 0; automated drift-check (`docs:cef-check`) deferred to Wave 1 | `docs:cef-check` implemented and green in CI |
