...
| Manuscript data in IndexedDB | AES-256-GCM for primary blobs and versioned secondary-record envelopes; configured-but-locked stores fail closed and legacy plaintext migrates lazily after unlock. Large LoRA weight blobs are excluded; their descriptive metadata and training records are encrypted. Cross-database passphrase rotation still requires a durable resumable journal. | `services/storage/storageEncryptionService.ts`, `docs/IDB-ENCRYPTION.md` |
...
- [x] Content-bearing secondary IDB encryption — scene revisions, inference results, ProForge memory/history, cross-project descriptions/embeddings, and LoRA metadata/datasets/runs use authenticated envelopes with locked/corrupt failure types and lazy legacy migration. Large LoRA weight blobs remain a documented exception.
- [ ] Resumable cross-database passphrase rotation — current rotation covers primary app data and snapshots only; add a durable journal before claiming complete or atomic rotation across secondary databases.
