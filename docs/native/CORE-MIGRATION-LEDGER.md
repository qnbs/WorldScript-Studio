...
| 4 | Logger / diagnostics | TS, `services/logger.ts` (312 lines) — dual-sink (IDB ring buffer + Tauri JSONL), plus Rust Core proof in `crates/worldscript-diagnostics` | Medium (dual-sink dispatch remains renderer-aware) | Medium (redaction chokepoint) | Low | Low | Medium-high — small, self-contained | 3 — Wave 2 fast-follow | **In progress — Core model/redaction locally proven; no authority switch** | Shared JSON fixtures are exercised independently by Rust and TS for top-level key redaction; Rust tests also cover non-mutation and serde wire shape; sink parity and production wiring remain open |
