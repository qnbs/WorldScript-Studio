...
disposition (§10.4.1) or introduce a new result code. **S5-B1 owns the identity-upgrade/recovery
procedure**, above: this S5-A baseline never claims Gate 5/7 admission for a `MIGRATE_TO_R15` class
whose only current representation returns `SOURCE_IDENTITY_UNBOUND` until S5-B1 defines that
procedure — never mutating/re-encrypting the frozen source to make this finding disappear in the
meantime. `WEBVIEW_IDB_AT_REST_V1` representation 2 keeps its existing AAD-bound identity.
...
**S5-B1 blocker — canonical migration source and payload evidence not yet admitted.** Two encodings are **not admitted by this S5-A baseline**: (1) `source_evidence_digest (LEGACY_PLAINTEXT)` (§5.4) is defined over exact frozen *file* bytes, but a packaged-desktop IndexedDB-fallback session persists structured-clone JS values via `compressData()` (`idbCore.ts`) — the raw value or an LZ-compressed UTF-16 string, chosen by a 10 KB threshold — neither a stable byte sequence that formula assumes; (2) `canonical_destination_payload_bytes` (§10.1.3's coalescing test, above) has no per-record-class codec for any `MIGRATE_TO_R15` class, so `source_value_digest` is unreproducible across independent Tauri/Qt adapters today. No implementation may invent a `JSON.stringify` ordering, a structured-clone/browser-serializer representation, source ciphertext bytes, or any other guessed canonicalization for either. Any authority switch requiring either digest remains blocked until the dedicated **S5-B1** Canonical Migration Source & Payload Evidence child contract admits both digests' canonical encodings, per-class destination-payload codecs, and the representation/version registries needed to reproduce them; `source_evidence_digest` and `source_value_digest` keep their existing, distinct meanings and are not redefined here.
...
Atomic-write temporary files are **not** in this no-disposition group: §3 already states their current unprotected content is plaintext/compressed and "can survive a crash," a genuine migration input/risk, not natively-created apparatus with no pre-existing content. **S5-B1 blocker — surviving atomic-write temporaries not yet reconciled.** A surviving `<target>.tmp-<UUID>` is not automatically junk: if it can contain a recoverable source value, it is a migration/recovery candidate for its owning logical record's authority, not its own disposition class. Until **S5-B1** admits deterministic reconciliation (discovery, owner derivation, candidate evidence, canonical-target comparison, duplicate/conflicting-candidate handling, cleanup eligibility): preserve every survivor, never delete or auto-promote one merely because its canonical target exists, never use mtime/newest-wins, never treat it as having no pre-existing source. Ambiguous owner/candidate state blocks the authority switch for that identity — the same `SOURCE_AUTHORITY_CONFLICT`-style posture §10.1.3 already uses for other unresolved multi-candidate state.
