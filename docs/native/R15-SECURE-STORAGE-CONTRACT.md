...
storage-root, record-local control, or explicit external-boundary records. A present `project_id` is
always the exact non-empty persisted project ID; an empty string is never a valid owner (§6.2). This is a scope rule,
...
**Absent vs. present-empty identities (version 1).** The three `project_id` states are distinct
and never collapse into one another:

```text
absent                  -> valid only for a class/identity with no project scope (§5.2.1); binding u8(0)
present, non-empty      -> valid; direct u8(1) or hashed u8(2) binding per rules A-E below
present, empty ("")     -> invalid; refused before AAD construction, never encoded as u8(1)+u32be(0)
                           and never treated as absent
```

The same rule applies to `logical_record_id`, which is always present: an empty logical identity is
refused. Any implementation that encodes an empty identity produces records a conformant reader
refuses, so this is a normative precondition, not an implementation choice.
...
1. **Core primitive and vectors:** implement the selected AEAD/KDF/randomness profile behind a
   headless interface; publish versioned vectors and malformed-input tests. Gate 1 is split in two
   because the KDF/key-provider profile is not yet selected:
   - **Gate 1a** — re-admitted by the QNB-100 readiness verdict (2026-09-26), its recorded admission
     authority: the `WSR1` envelope header and strict parser (§6.1/§6.1.2), the record-class registry
