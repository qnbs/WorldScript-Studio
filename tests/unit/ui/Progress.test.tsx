...
describe('Progress', () => {
  ...
  it('renders correctly at exactly 0', () => {
    const { container } = render(<Progress aria-label="Progress" value={0} />);
    const bar = container.querySelector('[style]') as HTMLElement;
    expect(bar.style.width).toBe('0%');
  });

  it('renders correctly at exactly 100', () => {
    const { container } = render(<Progress aria-label="Progress" value={100} />);
    const bar = container.querySelector('[style]') as HTMLElement;
    expect(bar.style.width).toBe('100%');
  });
  ...
  it('applies className to outer div', () => {
    const { container } = render(
      <Progress aria-label="Progress" value={50} className="my-custom-class" />,
    );
