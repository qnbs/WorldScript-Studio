...
// QNBS-v3 (Codex, PR #823; extracted to reduce evaluateBaseline's own complexity after CodeScene flagged it): Object.hasOwn, not bracket-index + `?? 0` — a ruleId present as an OWN key on one summary but absent from the other would otherwise read the OTHER summary's inherited Object.prototype property (e.g. ruleId "constructor" resolves to Object.prototype.constructor, not undefined), so `?? 0` never triggers and the numeric comparison silently evaluates false in both directions, hiding a real regression.
function ownCount(summary, ruleId) {
  return Object.hasOwn(summary, ruleId) ? summary[ruleId] : 0;
}
...
export function evaluateBaseline(audit, baseline) {
  ...
  for (const ruleId of allRuleIds) {
    const current = ownCount(audit.summary, ruleId);
    const baselined = ownCount(baseline.summary, ruleId);
