...
- ✅ **Dependabot batch — all 19 open PRs resolved** — 14 merged directly (Actions bumps, small JS/dev-tooling patches, Tauri/Rust crates, the tauri-deps group, and a stray `log` crate bump in `/src-tauri`, #246). The remaining 5 all had real breaking-change blockers, root-caused and fixed rather than force-merged or suppressed:
