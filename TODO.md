...
- ✅ **OpenRouter Settings Section hardening** (2026-06-13) — searchable design-system `Select`, model catalog fetch/cache, key validation/test-connection, full i18n (38 new keys × 11 locales), `ViewErrorBoundary` wrapping, 10 `OpenRouterSection` unit tests + 12 `openrouterModels` tests + 4 provider tests. All 7 CodeAnt AI inline comments addressed (free-model i18n, cloud AI policy gates, cache validation, Escape propagation). PR `feat/openrouter-section-perfection`.
