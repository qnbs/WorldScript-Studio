  ...
  switch (context.operation) {
    case 'enable': {
      ...
      if (isSecureRecordEnvelope(payload)) {
        const decoded = await readSecureRecordPayloadWithKey<Payload>(payload, secureContext, targetKey);
    ...
    case 'rekey': {
      ...
      try {
        const target = await readSecureRecordPayloadWithKey<Payload>(payload, secureContext, targetKey);
        ...
        try {
          const source = await readSecureRecordPayloadWithKey<Payload>(payload, secureContext, sourceKey);
    ...
    case 'disable': {
      ...
      const sourceKey = requireKey(context.sourceKey, 'source', spec.id);
      const decoded = await readSecureRecordPayloadWithKey<Payload>(payload, secureContext, sourceKey);
      if (!spec.isPayload(decoded.value)) throw new SecureRecordCorruptError();
...
export function createSecondaryPayloadStoreAdapter<Record extends object, Payload>(
  ...
  async function withDatabase<T>(operation: (database: IDBDatabase | null) => Promise<T>): Promise<T> {
    const database = await openExistingDatabase(spec.databaseName);
    try {
      if (database && !database.objectStoreNames.contains(spec.storeName)) return await operation(null);
  ...
  return {
    ...
    async migrateNext(context): Promise<ProtectedStoreMigrationBatch> {
      return withDatabase(async (database) => {
        ...
        await writeBatch(database, spec.storeName, rewritten);
    ...
    async verify(context): Promise<number> {
      return withDatabase(async (database) => {
        ...
        while (true) {
          ...
          if (!last) {
            throw new ProtectedStoreMigrationAdapterError(`${spec.id} verification made no progress`);
          }
