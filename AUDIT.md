...
**Current version:** **v1.28.3 released** — tag `v1.28.3` and the GitHub Release were published on 2026-08-27 from the validated `main` commit `a6e3ba3d`, a patch release fixing two unrelated issues: #527 (onboarding bootstrap-effect race that could skip the welcome portal for new users, originally found via v1.28.2's own tag-run flake) and #532 (a separate E2E test-harness precondition gap found on the post-merge `main` run for PR #530, unrelated to the already-fixed portal race; issue #532's root cause stays open — the fix makes `export.spec.ts` deterministic regardless of it). **7183+ Vitest source-declared tests across 589 test files · 2925 keys × 19 locales**. Historical release sections below remain historical.
