...
- **Dependabot hardening:** added `cooldown: default-days: 3` to all three `.github/dependabot.yml` ecosystems so newly released versions age 3 days before a PR is opened (the dependabot.yml equivalent of the `.npmrc` `minimum-release-age` already set for pnpm install-time). Open queue (#150/#151 Rust candle green; #152 dev-tooling + #154/#155 npm failing) handled per the CodeAnt Correction Loop.
