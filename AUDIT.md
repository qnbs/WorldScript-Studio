...
**Current version:** **v1.28.7 release candidate (pre-tag)** — the release cut is being prepared from the verified post-#802 `main` commit `a6397a97` under the explicit one-time AI/local-AI qualification exception documented in #743. This is not a release/tag claim: no `v1.28.7` tag, GitHub Release, desktop asset, updater manifest, or publication exists yet. #743 remains **OPEN / NOT PASS**; its incomplete real-runtime and hardware qualification continues post-release. The release still requires exact version synchronization, protected release-PR CI/CodeQL/security gates, Tauri plugin parity, and pre-tag Ubuntu/Windows/macOS ARM desktop qualification. **8119+ Vitest source-declared tests across 616 test files · 2942 keys × 19 locales**. Historical release sections below remain historical.
