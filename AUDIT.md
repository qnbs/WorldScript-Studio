...
symlink components (plus a fail-closed duplicate-entry guard).
...
**Current version:** **v1.28.8, published** — tag, GitHub Release, and full desktop asset set are live. Carries the Desktop Safe Open startup-recovery fix (#804/#807). #743 remains **OPEN / NOT PASS**; its qualification tracker continues post-release under the durable governance reconciliation (2026-09-22, recorded on #743) that separates that tracker from the concrete Release invariant, which was re-verified clean for this release — not a reused v1.28.7-only exception. **8147+ Vitest source-declared tests across 617 test files · 2944 keys × 19 locales**. Historical release sections below remain historical.

**Release gate (2026-09-22 — v1.28.8, published):** Resulting-main CI/CD run `35707720916` and CodeQL passed for the release-prep merge commit `868d66bc` immediately after PR #808 merged. Pre-tag exact-SHA Tauri qualification: `workflow_dispatch` run `35710294394` on `868d66bc` — Ubuntu/Windows/macOS bundle jobs all SUCCESS. Real-artifact Linux Safe Open runtime check against the built `.AppImage` (isolated `XDG_*` profile, `--appimage-extract-and-run`, headless Xvfb, real click-through via XTest — not mocked): a `FUTURE`-schema fixture and a distinct `UNSUPPORTED_OLDER`-schema fixture each rendered their correct, distinct recovery copy; clicking "Open WorldScript Studio without this project" transitioned cleanly to the portal with no "Continue" option offered; the refused project's file hash was verified byte-identical before and after; the active-project marker stayed pointed at the refused project until a new blank project was created and successfully written, at which point it advanced to that new project's fresh session-minted identity (`project-<uuid>`) only, and the new project's own auxiliary writes (`codex/codex.snap`, `codex/vectors.snap`) landed under that same new identity — the refused project's directory was never touched. Tag-triggered `v1.28.8` workflows: `Tauri desktop build` (`35717721039`) SUCCESS, `CI / CD` (`35717721094`) SUCCESS, `Docker` (`35717721112`) SUCCESS. GitHub Release `v1.28.8` is published, non-draft, non-prerelease, with the full expected asset set: Linux `.deb`/`.rpm`/`.AppImage` + `.sig`, Windows `.exe`/`.msi` + `.sig`, macOS ARM `.dmg` + updater `.app.tar.gz`/`.sig`, and `latest.json` (verified internally coherent: correct version, correct platform keys with no false Intel-macOS claim, correct asset URLs, real signatures). #743 remained OPEN / NOT PASS under the durable governance reconciliation, not a reused exception.

**Release gate (2026-09-21 — v1.28.7, published):** Main CI/CD run `35607774548` and CodeQL run `35596550205` passed for the resulting release-prep merge commit `c554ea71`. Tag-triggered `v1.28.7` workflows: `Tauri desktop build` (`35607774564`) SUCCESS, `CI / CD` (`35607774548`) SUCCESS, `Docker` (`35607774542`) SUCCESS. GitHub Release `v1.28.7` is published, non-draft, non-prerelease, with the full expected asset set: Linux `.deb`/`.rpm`/`.AppImage` + `.sig`, Windows `.exe`/`.msi` + `.sig`, macOS ARM `.dmg` + updater `.app.tar.gz`/`.sig`, and `latest.json`. Intel macOS was not claimed (deferred per the documented build matrix). #743 remained OPEN / NOT PASS under its one-time, v1.28.7-only maintainer exception (tracker-completeness timing waived only; no Release-invariant item was claimed satisfied by the exception itself — it was separately verified clean).
...
- **`pnpm run build` warnings eliminated:** an invalid `[dir:ltr]`/`[dir:rtl]` Tailwind arbitrary variant (compiled to the invalid CSS pseudo-class `:is(dir:ltr)`) replaced with Tailwind's built-in `ltr:`/`rtl:` direction variants; 3 dynamic `import()` calls Rolldown flagged as ineffective (already statically imported elsewhere) converted to static imports.
- **CodeRabbit review-body findings that failed to post inline** (GitHub API error on a rate-limited re-review of PR #274) verified against current code and fixed: `AiProviderCard` a11y (`role="status" aria-live="polite"`) and a genuine CWE-209 stale-request race in `handleTest` (monotonic request-id guard); `ollamaService.ts`'s `plugin_unavailable` branch no longer forwards `LocalServerError.message` directly.
...
| adm-zip | >=0.6.1 | GHSA-xcpc-8h2w-3j85 / GHSA-7q85-xj36-vmfc / GHSA-vwc7-r8mq-g2x9 | Transitive via `onnxruntime-node@1.24.3`; `0.6.1` is outside the vulnerable ranges, and its tarball adds the extraction symlink-path guard and duplicate-entry fail-closed behavior relevant to the old advisory. The 2026-09-11 release satisfies the 7-day minimum age; `pnpm audit` is clean and the `allowBuilds: onnxruntime-node: false` policy remains active. |
...
| qs | >=6.16.0 | GHSA-4mjr-xmp4-gh2g / GHSA-x5fp-wj9c-mxmx (supersedes GHSA-hrpp-h998-j3pp / CVE-2022-24999) | Array-limit bypass via bracket-key comma parsing, and a DoS via attacker-controlled `isBuffer` (resolved 6.15.2 vulnerable; fixed 6.16.0). Dev-only transitive (`@lhci/cli`, `@stryker-mutator/core`, `http-server`), never shipped. `6.16.0` was published 2026-08-29 and was admitted via a temporary, version-scoped `minimumReleaseAgeExclude: qs@6.16.0` entry while it aged inside this repo's 7-day `minimumReleaseAge` quarantine (same mechanism/precedent as `nanoid@3.3.18`, PR #362). That exclusion has since been removed (#588): `6.16.0` is now well past the 7-day floor and a normal frozen install resolves it unconditionally under the standard quarantine, with no version-scoped exception in effect. Future qs releases remain governed by the normal `minimumReleaseAge` quarantine like any other dependency. |
