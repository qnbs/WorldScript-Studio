...
**Current version:** **v1.28.8 release candidate (pre-tag)** — the release cut is being prepared from the verified post-#807 `main` commit, carrying the Desktop Safe Open startup-recovery fix (#804/#807). This is not a release/tag claim: no `v1.28.8` tag, GitHub Release, desktop asset, updater manifest, or publication exists yet. `v1.28.7` (below) is fully published. #743 remains **OPEN / NOT PASS**; its qualification tracker continues post-release under a durable governance reconciliation (2026-09-22, recorded on #743) that separates that tracker from the concrete Release invariant, which was re-verified clean for this candidate — not a reused v1.28.7-only exception. The release still requires exact version synchronization, protected release-PR CI/CodeQL/security gates, Tauri plugin parity, and pre-tag Ubuntu/Windows/macOS ARM desktop qualification. **8147+ Vitest source-declared tests across 617 test files · 2944 keys × 19 locales**. Historical release sections below remain historical.
