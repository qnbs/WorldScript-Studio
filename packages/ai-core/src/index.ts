...
async function runTransformersLayer(
  ...
  if (signal?.aborted) throw new Error('Aborted');
  const { pipeline, env } = await import('./vendor-transformers');
...
export async function runLocalTextGeneration(
  ...
  const MLC_TO_ONNX_MAP: Readonly<Record<string, string>> = {
    'Qwen2.5-0.5B': ONNX_SUPPORTED_MODELS[0]!.id,
    'Llama-3.2-1B': ONNX_SUPPORTED_MODELS[0]!.id,
    'gemma-3-1b': ONNX_SUPPORTED_MODELS[0]!.id,
    'Llama-3.2-3B': ONNX_SUPPORTED_MODELS[1]!.id,
  ...
  const resolvedOnnxModelId: string = (() => {
    ...
    return ONNX_SUPPORTED_MODELS[0]!.id;
  })();

  // Layer-2: ONNX WASM (no GPU required) — uses runTransformersLayer + AbortSignal
  try {
    const text = await runTransformersLayer(sanitizedPrompt, resolvedOnnxModelId, 128, 0.7, signal);
