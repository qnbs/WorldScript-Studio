...
**Review-comment completeness — check three independent channels before declaring a PR review-clean, every time; only one has a resolve mechanism:** (1) `UNRESOLVED_REVIEW_THREADS = 0` — GraphQL `reviewThreads` for inline per-line comments, the *only* channel resolved via `resolveReviewThread`; (2) `NO_ACTIONABLE_TOP_LEVEL_ISSUE_COMMENTS = YES` — `gh api repos/<owner>/<repo>/issues/<PR>/comments` for plain top-level bot comments (qodo-code-review posts its real findings only here, never as `reviewThreads`) — no resolve mutation exists, inspect/dispose/reply instead; (3) `NO_ACTIONABLE_REVIEW_BODY_FINDINGS = YES` — `gh api repos/<owner>/<repo>/pulls/<PR>/reviews`, reading each review's full `.body` text (CodeRabbit's "🧹 Nitpick comments" and outside-diff-range findings live here, collapsed, invisible to the other two channels) — same treatment, no resolve mutation. A bot using one channel on a PR doesn't mean the others are covered, and reviewer channel usage is observed live per PR, not fixed by this roster.
...
**Known review bots on this repo** (confirm still installed — this list can drift): CodeRabbit (automatic incremental review on push is the normal path; `@coderabbitai review` only when auto-review is paused/inapplicable — it does not re-review already-consumed commits, see `docs/PR-CI-MERGE-WORKFLOW.md`), CodeAnt AI (may expose 5 CI status checks — `CodeAnt - Quality Gates/SAST/SCA/SCR/Test Coverage` — and may also post a genuine inline review thread, confirmed 2026-08-28 on PR #538; inspect its actual current output rather than assuming from this entry), qodo-code-review (top-level comments, see above), Amazon Q Developer (`/q review` as a fresh top-level comment — not inside an existing thread; quota-conscious — call it once CodeRabbit/CodeAnt's own loop has already reached quiescence, not after every fix commit), Graphite AI Reviews (automatic, no confirmed manual trigger), chatgpt-codex-connector (intermittent/quota-limited availability — verify it's currently active rather than assuming silence means "nothing to report"). A bot's silence is not a clean pass by itself — for security/sandbox/IPC/FFI/packaging-adjacent PRs, verify at least one bot produced real review output (its actual comment/review text), not just a green check-run.
...
| `docs/PR-CI-MERGE-WORKFLOW.md` | Canonical overall PR/CI/merge lifecycle — CodeRabbit trigger semantics, merge-readiness classification, auto-merge mechanics |
| `docs/CODEANT-REVIEW-LOOP.md` | Review-comment reconciliation sub-procedure (any bot) — overall lifecycle authority is `PR-CI-MERGE-WORKFLOW.md` |
